...
\texttt{spring-petclinic} application, \Cref{sec:eval-rootcause} analyses
\emph{why} the text-editing baseline fails the cases it fails, and
\Cref{sec:eval-commons} replicates the comparison on a larger independent project
(Apache Commons Lang). \Cref{sec:eval-controlled} then holds the driving model
fixed and varies only the edit primitive across all three real-world projects.
...
\begin{description}
  \item[RQ1 (Correctness).] Does constraining the agent's action space to the
    ...
    refactorings on real-world codebases, and where does a text-substitution agent
    on the same tasks fail? \emph{Answered by the compile-and-content validation
    across all benchmark projects
    (\Cref{sec:eval-sample,sec:eval-petclinic,sec:eval-commons,sec:eval-controlled})
    and the root-cause analysis (\Cref{sec:eval-rootcause}). The agent arms are
    driven blind --- given only the task description --- so these runs also show
    that a function-calling LLM can locate symbols and select operations
    autonomously.}
  \item[RQ2 (Necessity).] For which classes of refactoring is a structured action
    space not merely beneficial but \emph{necessary} --- i.e.\ where does
    text-level editing fail for reasons intrinsic to its action representation
    rather than to model capability? \emph{Answered by the root-cause analysis
    (\Cref{sec:eval-rootcause}) and the controlled comparison
    (\Cref{sec:eval-controlled}), which isolates the failures that persist when the
    driving model is held fixed.}
  \item[RQ3 (Cost).] What does routing every edit through the IDE engine cost ---
    in the minimality of the resulting diffs and the agent's interaction cost (tool
    calls and turns) --- relative to a text-editing agent? \emph{Answered by the
    efficiency measurements (\Cref{sec:eval-sample}) and the diff-minimality
    analysis (\Cref{sec:eval-diff}).}
\end{description}
...
\begin{table}[t]
  ...
  the tool-layer ceiling (direct execution, no LLM); the LLM-driven structured agent ---
  Claude Opus~4.8 selecting and invoking the same operations --- is reported in the
  controlled comparison (\Cref{sec:eval-controlled}).}
  \label{tab:petclinic}
  \begin{tabular}{@{}llccc@{}}
    \toprule
