...
A growing body of work evaluates how well LLMs can refactor code;
\citet{aicodeopt2025} survey this landscape across machine-learning,
reinforcement-learning, and code-model techniques and identify trust and
explainability as the principal barriers to adoption in production --- barriers
that a structurally-safe, behaviour-preserving execution layer is well placed to
lower.
\citet{cordeiro2024refactoring} study StarCoder2, GPT-4o, GPT-4o-mini, LLaMA~3
and DeepSeek-v3 across thirty open-source Java projects, and find that LLMs
outperform developers on systematic, repetitive refactorings but fall short on
context-heavy, architectural ones --- precisely the cases that depend on
project-wide reference information. \citet{potentialllm2024} examine 180
...
self-repair for extract-method, with 82.8\% of outputs compiling and passing
tests; the pipeline is sophisticated but still produces text-level
replacements rather than driving the IDE's engine.
A distinct, human-centred strand is exemplified by \citet{interactiveagent2025},
whose three-layer agent detects smells, ranks candidate refactorings, and
negotiates them with the developer through a natural-language interface,
emphasising intent, project context, and trust. That work and the present one are
complementary: it concerns \emph{how an agent should decide and communicate} which
refactoring to propose, whereas this project concerns \emph{how the chosen
refactoring should be executed} so that the result is correct by construction
regardless of which party --- human or model --- selected it.
...
\citet{demystifying2025} provide a systematic taxonomy of LLM software-engineering
agents by tool repertoire, memory, action space, and coordination, and surface
the central tension between \emph{generality} (a Turing-complete shell) and
\emph{safety} (a constrained action space). This framing positions the present
work: rather than exposing a general shell, the plugin offers only typed,
